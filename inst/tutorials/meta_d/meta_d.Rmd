...
* [brewin_es.csv](https://milton-the-cat.rocks/data/csv/brewin_es.csv)
...
- Each study is uniquely identified by variable (`author` and `study`)
...
Where the relevant descriptive information is available the formula in @mathur_simple_2020 can be used to convert a standard Pearson correlation coefficient into $\hat{d}$.
...
It also possible to fit the model using robust estimates of the variances and covariances by placing the model into `metafor::robust()`. This function takes the form
...
8. We can set the intervals on the *x*-axis using `at`. The effect size data range from -1 to 2, so an interval of 0.5 might be good. If we include `at = seq(-1, 2, 0.5)` this will create ticks between -1 and 2 at intervals of 0.5 (that is, -1, -0.5, 0, 0.5, 1, 1.5, and 2).
...
If we place `foa_gp` into the model then `r rproj()` will convert this variable to 3 dummy variables that compare each category to the first. Let's check which category is the first.

#### `r robot()` Code example

Use the code box to check the order of the factor levels of the variable `foa_gp`
...
levels(brewin_es$foa_gp)
```


You should find that the groups are currently ordered: FOA (detailed), FOA (global),  not FOA (disorganisation), not FOA (organisation). In fact this is what we want: we want everything to be compared to the first category, which is the detailed FOA methodology.

<div class="tip">
  `r cat_space()` **Tip**

  If you want to compare each category to a category that is not currently the first category, you can use `forcats::fct_relevel()` to move the reference category to be the first level/category of your factor. The default behaviour of this function is to move the specified category to be first. For example, let's imagine we wanted to compare each category to "Not FOA (Organisation)" because we perceive this to be the worst way to measure memory disorganization. We could re-order the categories of `foa_gp` to achieve this:


```{r, eval = F, class.source = '.panel_alt'}
brewin_es <- brewin_es |>
  dplyr::mutate(
    foa_gp = forcats::fct_relevel(foa_gp, "Not FOA (Organisation)")
  )
```

This code uses `mutate()` to change the variable `foa_gp` so that it is a version of itself in which the specified factor level ("Not FOA (Organisation)") becomes the first level. Be careful of typos when you specify the factor level!
</div>
...
and so on. We want to add in the variable `foa_gp` so the `mods` argument will be
...
To add `gp_foa` as a moderator to our model we would take the code we used to fit the model without the moderator:
...
The overall moderation effect was significant, `r metahelpr::report_mod(brewin_rob)`. The parameter estimates (Table 2) indicated that compared to effect sizes relating to FOA studies that use detailed coding, effect sizes were (1) significantly larger in FOA studies that use global ratings, `r metahelpr::report_pars(brewin_rob, row = 2)`; (2) significantly smaller in not FOA studies that measured organisation, `r metahelpr::report_pars(brewin_rob, row = 3)`; and (3) not significantly different to not FOA studies that measured disorganisation, `r metahelpr::report_pars(brewin_rob, row = 4)`.

</div>

```{r, echo = F}
metahelpr::report_par_tbl(brewin_rob, pred_labels = c("Intercept", "FOA global vs. FOA detailed", "Not FOA (disorganisation) vs. FOA detailed", "Not FOA (organisation) vs. FOA detailed")) |> 
  knitr::kable(caption = "Table 2: Parameter estimates for the method of measuring memory disorganisation",
...
Table 2 takes advantage of `metahelpr::report_par_tbl()`, which is a helper function for reporting parameter estimates in a  nicely formatted table. I used the following code:

```{r, eval = F, class.source = '.panel_alt'}
metahelpr::report_par_tbl(brewin_rob,
...
In the code box below change produce the same plot but for the data in `brewin_es` (where effect sizes have not been aggregated) and see the difference.
