...
If we just place `foa_gp` into the model then this variable will automatically be converted to dummy variables that compare each category to the first. In fact this is what we want. 

<div class="tip">
  `r cat_space()` **Tip**

  If you want to compare each category to a category that is not currently the first category, you can use `forcats::fct_relevel()` to move the reference category to be the first level/category of your factor. The default behaviour of this function is to move the specified category to be first. Fore xample, let's imagine we wanted to compare each category to "Not FOA (Organisation)" because we perceive this to be the worst way to measure memory disorganization. We could re-order the categories of `foa_gp` to achieve this:
...
```{r eval = F}
metahelpr::get_mas(tibble = name_your_tibble,
                   predictor = name_of_moderator,
                   es = variable_containing_effect_sizes,
                   var_es = variable_containing_effect_size_variances,
                   es_id = variable_containing_effect_size_identifier, # optional
                   study_id = variable_containing_cluster_identifier, # optional
                   digits = decimal_places_to_use_in_table # default is 2
                   p_digits = decimal_places_for_p_values #default is 3
